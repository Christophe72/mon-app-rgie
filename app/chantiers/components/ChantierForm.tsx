...
const INPUT =
  "bg-gray-800 rounded-lg px-3 py-2 text-white outline-none w-full text-sm";
...
export default function ChantierForm({
  ...
  const [statut, setStatut] = useState<StatutChantier>(
    initial?.statut ?? "EN_ATTENTE",
  );
  const [datePrevue, setDatePrevue] = useState(initial?.datePrevue ?? "");
  const [typeSupport, setTypeSupport] = useState<TypeSupport[]>(
    initial?.typeSupport ?? ["BLOC"],
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-gray-900 rounded-xl p-4 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="Ex : Rue des Lilas 12"
            className={INPUT}
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            placeholder="Nature des travaux, contexte..."
            rows={2}
      ...
      <div className="bg-gray-900 rounded-xl p-4 space-y-1.5">
        <label className={LABEL}>
          Contraintes{" "}
          <span className="normal-case text-gray-600">(optionnel)</span>
        </label>
        <textarea
          value={contraintes}
